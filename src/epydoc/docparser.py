...
def process_file(module_doc):
    ...
    for toktype, toktext, (srow,scol), (erow,ecol), line_str in tok_iter:
        ...
        # comment block and the thing it describes.
            ...
            if comments and not line_toks and last_toktype != tokenize.COMMENT:
...
def lookup_name(identifier, parent_docs):
    ...
    # Builtins
    builtins_docs = epydoc.docintrospecter.introspect_docs(builtins)
    if isinstance(builtins_docs, NamespaceDoc):
        if identifier in builtins_docs.variables:
            return builtins_docs.variables[identifier]
